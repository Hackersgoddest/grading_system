...
{ // Getting student total score from the user and storing it in the student number grade array variable
    ...
    do
    {
        ...
        is_digit(score);
        num = stof(score); // Converting string variable "score" to double with the help of the "stod" function and storing it in student num grade array
        clearError();
        placeCursor(screen, 14, 25);
        if (num < 0.0)
  ...
  //   flag = 0; // Intentionally initializing flag to zero to check whether the names of the students have been sorted
    ...
    while (flag == 1)
    {
        ...
        while (x <= index - 2)
        {
            ...
            string iTemp;
            float nGrade;
            if (StudentName[x] > StudentName[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1])
            {
                ...
                fTemp = StudentName[x],
                    ...
                    StudentName[x] = StudentName[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1],
                    StudentId[x] = StudentId[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1],
                    StudentNumGrade[x] = StudentNumGrade[static_cast<std::vector<float, std::allocator<float>>::size_type>(x) + 1],
                    StudentLetterGrade[x] = StudentLetterGrade[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1],
                    StudentName[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1] = fTemp,
                    StudentId[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1] = iTemp,
                    StudentNumGrade[static_cast<std::vector<float, std::allocator<float>>::size_type>(x) + 1] = nGrade,
                    StudentLetterGrade[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(x) + 1] = lGrade,
                    flag = 1;
            }
            x++;
        }
